    ...
    public class HandVisualizer : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            else if (m_hand.GetTrackingState() == ARTrackable.TrackingState.TRACKING)
            {
                //_UpdateHandBox();
                //_UpdateHandSkeleton();
                //Debug.LogError("Time " + System.DateTime.Now.Millisecond + " lasttime "+lastShotTime);
                Debug.LogError("MANA :"+ PlayerScript.getMana());
                var now = System.DateTimeOffset.Now.ToUnixTimeSeconds();
                if (
                    now - lastShotTime >= 1
                    )
                {
                    if (m_hand.GetGestureType() == 0 && PlayerScript.getMana() >= 25)
                    {
                        lastShotTime = now;
                        shootFireball();
                    } else if(m_hand.GetGestureType() == 6 && PlayerScript.getMana() >= 30)
                    {
                        lastShotTime = now;
                        shootToxicball();

                    } else if (m_hand.GetGestureType() == 1 && PlayerScript.getMana() >= 35)
                    {
                        lastShotTime = now;
                        shootElectricball();
                    }
            ...
            PlayerScript.incrementManaBy(1 / 6f);
        }

        private void shootFireball()
        {
            Instantiate(fireball, m_handCamera.transform.position, m_handCamera.transform.rotation);
            PlayerScript.decrementManaBy(25);
            audioSource.PlayOneShot(fireClip);
        }

        private void shootToxicball()
        {
            Instantiate(toxicball, m_handCamera.transform.position, m_handCamera.transform.rotation);
            PlayerScript.decrementManaBy(30);
            audioSource.PlayOneShot(toxicClip);
        }
        private void shootElectricball()
        {
            Instantiate(electricball, m_handCamera.transform.position, m_handCamera.transform.rotation);
            PlayerScript.decrementManaBy(35);
            audioSource.PlayOneShot(electricClip);
        }
